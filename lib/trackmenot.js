...
TRACKMENOT.TMNSearch = function() {
    ...
    function runScript(tab) {
        ...
        worker_opt.port.on("TMNOptionsShowLog", sendLogToPanel)
        worker_opt.port.on("TMNOptionsClearLog", clearLog)
    }
	
    function sendLogToPanel() {
   ...
   function sendOptionToTab() {
    ...
    function saveOptionFromTab(options) {
        ...
        cout("Searched engines: "+ searchEngines)
    ...
    function monitorBurst() {
       pageMod.PageMod({
          ...
          onAttach: function onAttach(worker) {
            ...
            cout("Worker find a match for url: "+ url)
            ...
            if (worker.tab != tmn_tab ) {
                enterBurst ( eng )
                cout("Entering burst mode")
            } 
    ...
    function updateOnSend ( queryToSend ) {
        ...
        cout("TMN query received from update is now: "+ queryToSend )
    ...
    function updateCurrentURL(taburl) {		  
        ...
        cout("currentTMNURL is :"+currentTMNURL)     
    }
    ...
    function handleRequest(request, sender, sendResponse) {
        ...
        switch (request.tmn) {
            case "currentURL":
                ...
                worker_tab.port.emit("TMNCurrentURLRes",response)
                break;
            case "useTab" :
                var response = {tmnUseTab: useTab}; 
                worker_tab.port.emit("useTabRes",response)              
                break;
            ...
            case "isActiveTab":
                ...
                cout("active: "+ active)
                worker_tab.port.emit("isActiveRes", {
                    isActive: active
                });       
                break;
            default:
...
return {
    ...
    updateCurrentURL: function (taburl) {
        currentTMNURL = taburl;       
    },
