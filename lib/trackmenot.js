...
TRACKMENOT.TMNSearch = function() {
   ...
   function sendOptionToTab() {
    ...
    function randomQuery()  {
        var qtype = typeoffeeds[ Math.floor(Math.random()*typeoffeeds.length)]
        var queries = [];
        cout(qtype)
        if ( qtype != 'zeitgeist' && qtype!='extracted') {
          var queryset = TMNQueries[qtype];
          queries = queryset[Math.floor(Math.random()*queryset.length)].words;
        } else queries = TMNQueries[qtype];
        var queryIdx = Math.floor(Math.random()*queries.length -0.1);
