...
TRACKMENOT.TMNSearch = function() {
    ...
    var init_engines = [
        {'id': 'google', 'name': 'Google Search', 'urlmap': "https://www.google.com/search?hl=en&q=|", 'regexmap': "^(https?:\/\/[a-z]+\.google\.(co\\.|com\\.)?[a-z]{2,3}\/(search){1}[\?]?.*?[&\?]{1}q=)([^&]*)(.*)$", "host": "(www\.google\.(co\.|com\.)?[a-z]{2,3})$", "testad": "var testad = function(ac,al,apc,as) {return ( apc && as && al && (ac=='l'  || ac=='l vst' || apc== 'r' ) && as.indexOf('return rwt') ==0 && al.indexOf('http')==0 && al.indexOf('https')!=0);}", 'box': SearchBox_google, 'button': getButton_google},
...
if (usingAustralis) {
  ...
  widget = Widget({
    ...
    function iniTab(tab) {
        ...
        tab.on("activate", function() {
            notifications.notify({
                ...
                iconURL: data.url("images/skin/tmn_icon_16.png")
            });
        });
    ...
    function scheduleNextSearch(delay) {
        ...
        cout("Time till next search: "+ tmn_timeTillNextSearch)
    }
    ...
    function saveOptions() {
        ...
        console.error("TMN options: "+ss.storage.options_tmn)
    ...
    function restoreOptions() {
        ...
        try {
            ...
            console.error("options found: "+ss.storage.options_tmn)
