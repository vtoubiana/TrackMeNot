...
TRACKMENOT.TMNSearch = function() {
    ...
    function iniTab(tab) {
        ...
        worker.port.on("TMNRequest",TACKMENOT.TMNSearch.handleRequest);
        worker.port.on("TMNUpdateURL",TACKMENOT.TMNSearch.updateCurrentURL);
        worker.port.on("close", TRACKMENOT.TMNSearch._preserveTMNTab); 
    ...
    function createTab() {
        if (!useTab || tmn_tab_id != -1) return null;
        ...
        handleRequest :  function(request, sender, sendResponse) {
            ...
            switch (request.tmn) {
                case "currentURL":
                    ...
                    self.port.emit("TMNCurrentURLRes",response)
                    break;
                case "useTab" :
                    var response = {tmnUseTab: useTab}; 
                    self.port.emit("useTabRes",response)              
                    break;
