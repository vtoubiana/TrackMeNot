...
TRACKMENOT.TMNSearch = function() {
    ...
    function sendQueriesToOption() {
        worker_opt.port.emit("TMNSendQueries",{queries:genQueries.join(',')})
    }
   ...
   function sendOptionToTab() {
    ...
    function saveOptionFromTab(options) {
        ...
        kwBlackList = options.kw_black_list;
    ...
    function getTMNTab() {
    debug("Tryinng to access to the tab")
    ...
    function createTab() {
        if (!useTab || getTMNTab()!= null) return getTMNTab();
        ...
        try {
            tabs.open({
                ...
                onOpen: iniTab,
                onReady : updateTab,
                onClose: TRACKMENOT.TMNSearch._preserveTMNTab
            });
        } catch (ex) {
    ...
    function extractQueries(html)    {
        ...
        if ( genQueries.indexOf(queryToAdd)<0 && !isBlackList(queryToAdd) )
          genQueries.push( queryToAdd );
      }
    ...
    function isBlackList( term ) {
     ...
     function addQuery(term, queryList, idx, checkSkipEx) {
   ...
   function sendQuery(queryToSend)  { 
    ...
    function saveOptions() {
        ...
        ss.storage.gen_queries = genQueries.join(',');
    ...
    function restoreOptions () {
        ...
        try {
            ...
            if (ss.storage.gen_queries && ss.storage.gen_queries.lengh>0) genQueries = ss.storage.gen_queries.split(',');
            ...
            if (ss.storage.kw_black_list && ss.storage.kw_black_list.length > 0)  kwBlackList = ss.storage.kw_black_list.split(",");   
        } catch (ex) {
    ...
    function handleRequest(request, sender, sendResponse) {
        ...
        switch (request.tmn) {
            case "pageLoaded": //Remove timer and then reschedule;  
                prev_engine = engine;       
                timer.clearTimeout(tmn_errTimeout);
                if (Math.random() < 20.02) {              
                  timer.setTimeout(sendClickEvent , 1000);
                }
                reschedule();
