...
var engines = [
		{'id':'google','name':'Google', 'urlmap':"https://www.google.com/search?hl=en&q=|", 'regexmap':"^(https?:\/\/[a-z]+\.google\.(co\\.|com\\.)?[a-z]{2,3}\/search[\?]?.*?[&\?]{1}q=)([^&]*)(.*)$"} ,
		{'id':'yahoo','name':'Yahoo!', 'urlmap':"http://search.yahoo.com/search;_ylt=" +getYahooId()+"?ei=UTF-8&fr=sfp&fr2=sfp&p=|&fspl=1", 'regexmap':"^(http:\/\/[a-z.]*?search\.yahoo\.com\/search.*?p=)([^&]*)(.*)$"},
		{'id':'bing','name':'Bing', 'urlmap':"http://www.bing.com/search?q=|", 'regexmap':"^(http:\/\/www\.bing\.com\/search\?[^&]*q=)([^&]*)(.*)$"},
		{'id':'baidu','name':'Baidu', 'urlmap':"http://www.baidu.com/s?wd=|", 'regexmap':"^(http:\/\/www\.baidu\.com\/s\?.*?wd=)([^&]*)(.*)$"},
		{'id':'aol','name':'Aol', 'urlmap':"http://www.baidu.com/s?wd=|", 'regexmap':"^(http:\/\/[a-z0-9.]*?search\.aol\.com\/aol\/search\?.*?q=)([^&]*)(.*)$"}
	]
	...
	function addEngine(param) {
		...
		cout(kw_param)
		new_engine.regexmap = '^('+ map.replace(/\//g,"\/").replace(/\./g,"\.").split('?')[0] + "\\?[^&]*" +kw_param+")([^&]*)(.*)$"
		cout(new_engine.regexmap)
    ...
    function monitorBurst() {
        pageMod.PageMod({
            ...
            onAttach: function onAttach(worker) {
                ...
                if (!tmn_tab || worker.tab.index != tmn_tab.index ) {
                    cout("Worker find a match for url: "+ url + " on engine "+ eng +"!")
    ...
    function checkForSearchUrl(url) {
        ...
        if (result.length !=4 ){
		cout(result)
            if (result.length ==5 && eng.id == "google"  ) {
                result.splice(2,1);
			}
