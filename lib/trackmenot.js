...
var engines = [
    ...
    function sendQuery(queryToSend)  { 
        ...
        if (queryToSend == null){ 
            if (incQueries && incQueries.length > 0) 
                ...
                if (!queryToSend) cerr('sendQuery error! queryToSendis null')
                return;
            }
        }
            ...
            var req = Request({
                ...
                onComplete: function (response) { 
                    ...
                    if (response.status >= 200 && response.status<400) {
                        ...
                        if (engine.id != "baidu") extractQueries(response.text);
                    } else {
    ...
    function restoreOptions () {
        ...
        try {
            ...
            if (options.kw_black_list && options.kw_black_list.length > 0)  kwBlackList = options.kw_black_list.split(",");   
        } catch (ex) {
