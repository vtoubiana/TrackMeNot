...
if (usingAustralis) {
  ...
  widget = Widget({
    ...
    function getSearchBoxCurrentEngine() {
        ...
        console.log("Current engine is: "+ currentEngine.name)
        return currentEngine;
    }
	
	function getDefaultsearchEngine() {
		console.log("Check for default search engine")
		try {
			...
			if ( selected_engine ) {
				...
				if (init_engines.filter(function(a) {return a.id == sel_engine_name }).length ==1) {
					console.log("Find a match for the current engine "+ sel_engine_name)
					return sel_engine_name
				}
			}
			return "google";
		} catch (ex) {
			console.log("Can not find default search engine: " + ex)  
			return "google";
		}
	}
	
	function setSearchEngine() {
		console.log("Set default search engine")
		var searchservice = Cc["@mozilla.org/browser/search-service;1"].createInstance(Ci.nsIBrowserSearchService);
